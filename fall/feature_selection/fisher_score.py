...
def fisher_score(
    group_values: np.ndarray, group_counts: np.ndarray, overall_stdev: float, group_stdev: Optional[np.ndarray] = None
) -> float:
    ...
    if group_stdev is not None:
        ...
        for concept_i, concept_sigma in enumerate(group_stdev):
            sum_concept_variance += group_proportions[concept_i] * math.pow((concept_sigma), 2)
